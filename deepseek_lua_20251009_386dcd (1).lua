...
-- Teleport GUI functionality
local function createTeleportGUI()
    if teleportGUIActive then
        ...
        for _, player in pairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and isEnemy(player) then
                TeleportTab:CreateButton({
                    Name = "Teleport Behind: " .. player.Name,
                    Callback = function()
                        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                            ...
                            local behindPosition = targetCFrame.Position - targetCFrame.LookVector * 5
        ...
        -- Close teleport window if it exists
        if teleportWindow then
            teleportWindow:Destroy()
            teleportWindow = nil
        end
...
-- Create toggle for Teleport GUI
local TeleportGUIToggle = MainTab:CreateToggle({
    Name = "Teleport GUI",
    CurrentValue = teleportGUIActive,
    Flag = "TeleportGUIToggle",
    Callback = function(Value)
        teleportGUIActive = Value
        createTeleportGUI()
        Rayfield:Notify({
            Title = "Teleport GUI",
            Content = Value and "Opened" or "Closed",
            Duration = 1.5,
            Image = 4483362458,
            Actions = {
                Ignore = {
...
InfoTab:CreateLabel("Rivals Hack Suite v1.3")
...
InfoTab:CreateLabel("- Teleport GUI (2-second teleport)")
